...
parser.add_argument('--beta',type=float,default=6.0,help='regularization parameter for TRADERS')
...
out_dir = f'./Results/TRADES/{args.dataset}/{args.net}/beta_{args.beta}/{time.strftime("%m%d")}_{time.strftime("%H%M", time.localtime())}_seed{args.seed}_'
if args.name != '':
    out_dir += (args.name + '_')
...
def train(model, train_loader, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if not args.no_regular:
            ...
            final_loss = TRADES_loss(output, model(data), target, args.beta)  + float(args.alpha) * loss_var
        else:
            final_loss = TRADES_loss(output, model(data), target, args.beta)
